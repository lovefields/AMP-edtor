function dEditor(continer){
	...
	html += '</select>';
	html += '<button class="d-btn d-btn-img" title="Add img">img</button>';
	...
	html += '<button class="d-btn d-btn-view" title="Change view mode">V</button>';
	...
	html += '</select>';
	html += '<button class="d-btn-apply">Apply</button>';
	html += '</label>';
	...
	html += '<iframe src="/d-editor/textarea.html" id="d-editor-doc-wrap" class="act"></iframe>';
	html += '</div>';
	...
	this.submit = function() {
		...
		this.item.querySelector('#d-editor-doc-val').value = val;
	}
	...
	// ol type
	this.item.querySelector('.d-btn-list-type select').addEventListener('change', () => {
		var type = this.item.querySelector('.d-btn-list-type select').value;
		iframe.focus();
		iframe.contentWindow.olType(type);
	});

	// pre language
	this.item.querySelector('.d-btn-lang select').addEventListener('change', () => {
		var lang = this.item.querySelector('.d-btn-lang select').value;
		iframe.focus();
		iframe.contentWindow.preLang(lang);
	});

	// pre color
	this.item.querySelector('.d-btn-lang .d-btn-apply').addEventListener('click', () => {
		var lang = this.item.querySelector('.d-btn-lang select').value;
		iframe.focus();
		iframe.contentWindow.preColor(lang);
	});

	// view mode
	this.item.querySelector('.d-btn-view').addEventListener('click', () => {
		var h = this.item.querySelector('#d-editor-doc-val').hidden;
		var val = this.item.querySelector('#d-editor-doc-val').value;
		if(h == true){
			this.submit();
			this.item.querySelector('#d-editor-doc-val').hidden = false;
			this.item.querySelector('#d-editor-doc-wrap').hidden = true;
		}else{
			this.item.querySelector('#d-editor-doc-val').hidden = true;
			this.item.querySelector('#d-editor-doc-wrap').hidden = false;
			iframe.focus();
			iframe.contentWindow.unValue(val);
		}
	});
